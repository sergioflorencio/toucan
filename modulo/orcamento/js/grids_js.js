	...
	function TreeGrid(a){
		...
		var thead="<thead style=''><tr><td style='display: none;border-width: 1px;'></td>";
		//display: none;
		for(var x=0;x<nColunas;x++){
			if(a.columns[x].dataType=='number'){ align="text-align: right !important;"; }
			else{align="text-align: left !important;";}	
			
			var input_="<input type='text' style='border: 0px !important;width:100%;height:100% !important;text-align: right;background-color: transparent;' id='x_y' value='"+a.columns[x].headerText+"'  disabled/>"
			thead+="<th style='padding: 1px !important;border-width: 1px;"+align+"' class='uk-text-truncate' ><div style='height: 100%;width:"+a.columns[x].width+"  !important;  '>"+input_+"</div></th>";
		...
		function getTbody( parentID,padding_,nivel ){
			return data.filter(function(node){ return ( node.PID === parentID ) ; }).sort(function(a,b){return a.ID > b.ID}).map(function(node){
				...
				var table_tr= "<tr class='nivel' id='"+node.ID+"'><td style='display: none;border-width: 1px;'>"+nivel+"</td>";
				//display: none;
				var nColunas=a.columns.length;
				for(var y=0;y<nColunas;y++){
					var valor_=String(node[a.columns[y].key]);
						...
						if(node.status=="ativa"){
							if(a.columns[y].headerText=="historico"){
								var input_="<input type='text' ID ='"+node.ID+"_"+a.columns[y].headerText+"' class='historico' cod_conta='"+node.ID+"' data_='"+a.columns[y].headerText+"' style='width: 100%; border: 1px solid #ccc;background-color: transparent !important;text-align: right; font-size: 12px; height: 100% ! important;' id='' value=''/>"
							}else{
								var input_="<input type='text' ID ='"+node.ID+"_"+a.columns[y].headerText+"' class='x_y' cod_conta='"+node.ID+"' data_='"+a.columns[y].headerText+"' style='width: 100%; border: 1px solid #ccc;background-color: transparent !important;text-align: right; font-size: 12px; height: 100% ! important;' id='' value=''/>"
							}
						...
						if(y==0){
							table_tr+="<td style='width:"+a.columns[y].width+" !important;border-width: 1px;font-size: 12px;vertical-align: middle;"+align+"'><span style='padding-left:"+padding+"px; width:"+a.columns[y].width+" !important;' class='uk-text-truncate' >"+node[a.columns[y].key]+"</span></td>";
							
							
						}else{
							table_tr+="<td style='width:"+a.columns[y].width+" !important;padding: 1px !important;border-width: 1px;vertical-align: middle;"+align+"'><div style='padding-left:"+padding+"px; width:"+a.columns[y].width+" !important;height: 100% ! important;' class='uk-text-truncate' >"+input_+"</div></td>";
		...
		tbody="<tbody id='tbody_"+a.idGrid+"' style='max-height: inherit ! important;'>"+getTbody(-1,0,0)+"<tbody>";
		var table="<div style='' class='uk-overflow-container' ><table class='uk-table uk-table-condensed uk-table-hover'>"+thead+tbody+"</table></div>";
	...
	function TreeGrid_(a){
		...
		for(var x=0;x<nColunas;x++){
			...
			//var input_="<input type='text' style='border: 0px !important;width:100%;height:100% !important;text-align: right;background-color: transparent;' id='x_y' value='"+a.columns[x].headerText+"'  disabled/>"
			thead+="<th style='padding: 1px !important;border-width: 1px;"+align+"' class='uk-text-truncate' ><div style='height: 100%;width:"+a.columns[x].width+"  !important;  '>"+a.columns[x].headerText+"</div></th>";
