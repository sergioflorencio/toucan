	...
	<div class="uk-grid" style="padding-left: 10px;">
		...
		<?php 
			if(isset($_GET['pesquisa'])){
				$tabelas=new tabelas;$tabelas->listar_campanhas($_GET['pesquisa']);
			} 

			if(isset($_GET['id'])){
				
				if($_GET['id']=="novo"){$cod_campanha="";}else{$cod_campanha=$_GET['id'];}
				
				$cadastros=new cadastros;
				$cadastros->cad_campanhas($cod_campanha);
			}
		?>
		</div>		
	</div>
