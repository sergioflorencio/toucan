	...
	<div class="uk-grid" style="padding-left: 10px;">
		...
		<?php 
			if(isset($_GET['pesquisa'])){
				$tabelas=new tabelas;$tabelas->listar_carteiras($_GET['pesquisa']);
			} 

			if(isset($_GET['id'])){
				
				if($_GET['id']=="novo"){$cod_carteira="";}else{$cod_carteira=$_GET['id'];}
				
				$cadastros=new cadastros;
				$cadastros->cad_carteiras($cod_carteira);
			}
		?>
		</div>		
	</div>
