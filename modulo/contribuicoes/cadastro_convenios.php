	...
	<div class="uk-grid" style="padding-left: 10px;">
		...
		<?php 
			if(isset($_GET['pesquisa'])){
				$tabelas=new tabelas;$tabelas->listar_convenios($_GET['pesquisa']);
			} 

			if(isset($_GET['id'])){
				
				if($_GET['id']=="novo"){$cod_convenio="";}else{$cod_convenio=$_GET['id'];}
				
				$cadastros=new cadastros;
				$cadastros->cad_convenios($cod_convenio);
			}
		?>
		</div>		
	</div>
