		...
		if(isset($_POST['cadastro']) and isset($_POST['valor'])){
			...
			class pesquisa_cod_{
				function cod_conta($numero_conta){
					...
					include "config.php";
					$select="SELECT cod_conta FROM ".$schema.".cad_conta where status='ativa' and numero_conta='".$numero_conta."' and cod_empresa=".$_SESSION['cod_empresa']." ; ";
				...
				function cod_centro_custo($numero_centro_custo){
					...
					include "config.php";
					$select="SELECT cod_centro_custo FROM ".$schema.".cad_centro_custo where status='ativa' and numero_centro_custo='".$numero_centro_custo."' and cod_empresa=".$_SESSION['cod_empresa']." ; ";
					$resultado=mysql_query($select,$conexao) or die (mysql_error());
					if(mysql_num_rows($resultado)>=1){return true;}else{return false;}
